...
function createSystemCard(key, data) {
    ...
    card.innerHTML = `
        ...
        <div class="switch-controls">
            <button class="switch-button switch-path-normal" onclick="switchToPath('${key}', 'normal')" id="switch-normal-${key}" disabled>🔄 Jalur Normal</button>
...
//     const res = await fetch(`http://${ip}/status?system=1`, {
//       method: 'GET',
//       mode: 'cors',
...
function updateESPStatus() {
    ...
    // Tambahan: Matikan tombol switch jika koneksi terputus
    if (!esp1Connected || !esp2Connected) {
        disableAllSwitchButtons();
    }
}
...
const CONNECTION_CHECK_INTERVAL = 3000; // misal 3 detik
...
//     if (!esp1Connected && !esp2Connected) {
//         showNotification('ESP belum terkoneksi', 'error');
//         return;
//     }
...
//     if (esp1Connected) promises.push(sendSwitchCommand(ip1, key, relay));
//     if (esp2Connected) promises.push(sendSwitchCommand(ip2, key, relay));

//     const result = await Promise.allSettled(promises);
//     //const result = await Promise.all(promises.map(p => p.catch(e => ({ error: e }))));
...
//     } else {
//         showNotification(`${name} gagal dialihkan`, 'warning');
//     }
// }
...
// async function sendSwitchCommand(ip, systemKey, relay) {
//     const systemIndex = systemsData[systemKey].index;

//     console.log(`Kirim ke ESP ${ip} => system=${systemIndex}&relay=${relay}`); // opsional debug


//     const res = await fetch(`http://${ip}/switch`, {
//         method: 'POST',
//         mode: 'cors',
//         headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
//         body: `system=${systemIndex}&relay=${relay}`
//     });
